...
public class MusicSampleFloor {

   static final int[] ranges;
   static final float[] decibelStaticTable;
   static int[] highNeighbor;
   static int[] lowNeighbor;
   static boolean[] step2Flags;
   int[] alXList;
   int multiplier;
   ...
   MusicSampleFloor() {
      ...
      if(type != 1) {
         ...
         this.classDimensions = new int[maximumClass];
         this.classSubClasses = new int[maximumClass];
         this.classMasterbooks = new int[maximumClass];
         this.subclassBooks = new int[maximumClass][];
         ...
         for(rangeBits = 0; rangeBits < maximumClass; ++rangeBits) {
            ...
            int[] var7 = new int[partitionClass];
         ...
         for(j = 0; j < partitions; ++j) {
            partitionClass += this.classDimensions[this.partitionClassList[j]];
         }

         this.alXList = new int[partitionClass];
         this.alXList[0] = 0;
         this.alXList[1] = 1 << rangeBits;
         partitionClass = 2;
         ...
         if(highNeighbor == null || highNeighbor.length < partitionClass) {
            highNeighbor = new int[partitionClass];
            lowNeighbor = new int[partitionClass];
            step2Flags = new boolean[partitionClass];
         }
   ...
   void method2365(int var1, int var2, int var3, int var4, float[] var5, int var6) {
      ...
      var5[var1] *= decibelStaticTable[var2];
      if(var3 > var6) {
         var3 = var6;
      }
      ...
      for(int var14 = var1 + 1; var14 < var3; ++var14) {
         ...
         var5[var14] *= decibelStaticTable[var11];
      }
   ...
   void method2366(int var1, int var2) {
      if(var1 < var2) {
         int var3 = var1;
         int var4 = highNeighbor[var1];
         int var5 = lowNeighbor[var1];
         boolean var6 = step2Flags[var1];

         for(int var7 = var1 + 1; var7 <= var2; ++var7) {
            int var8 = highNeighbor[var7];
            if(var8 < var4) {
               highNeighbor[var3] = var8;
               lowNeighbor[var3] = lowNeighbor[var7];
               step2Flags[var3] = step2Flags[var7];
               ++var3;
               highNeighbor[var7] = highNeighbor[var3];
               lowNeighbor[var7] = lowNeighbor[var3];
               step2Flags[var7] = step2Flags[var3];
            }
         }

         highNeighbor[var3] = var4;
         lowNeighbor[var3] = var5;
         step2Flags[var3] = var6;
   ...
   boolean decodeFloor() {
      ...
      if(!var1) {
         ...
         int var2 = this.alXList.length;

         int var3;
         for(var3 = 0; var3 < var2; ++var3) {
            highNeighbor[var3] = this.alXList[var3];
         }
         ...
         for (int var7 : this.partitionClassList) {
            ...
            int var11 = 0;
            if (var9 > 0) {
               var11 = MusicSample.codebooks[this.classMasterbooks[var7]].getHuffmanRoot();
            }

            for (int var12 = 0; var12 < var8; ++var12) {
               int var13 = this.subclassBooks[var7][var11 & var10];
               var11 >>>= var9;
               lowNeighbor[var5++] = var13 >= 0 ? MusicSample.codebooks[var13].getHuffmanRoot() : 0;
            }
         }
   ...
   void computeFloor(float[] var1, int var2) {
      ...
      for(var6 = 2; var6 < var3; ++var6) {
         var7 = method2369(highNeighbor, var6);
         var8 = method2363(highNeighbor, var6);
         var9 = this.method2379(highNeighbor[var7], lowNeighbor[var7], highNeighbor[var8], lowNeighbor[var8], highNeighbor[var6]);
         var10 = lowNeighbor[var6];
         int var11 = var4 - var9;
         int var12 = (Math.min(var11, var9)) << 1;
         if(var10 != 0) {
            boolean[] var13 = step2Flags;
            step2Flags[var8] = true;
            var13[var7] = true;
            step2Flags[var6] = true;
            if(var10 >= var12) {
               lowNeighbor[var6] = var11 > var9?var9 + (var10 - var9):var11 + (var9 - var10) - 1;
            } else {
               lowNeighbor[var6] = (var10 & 1) != 0?var9 - (var10 + 1) / 2:var10 / 2 + var9;
            }
         } else {
            step2Flags[var6] = false;
            lowNeighbor[var6] = var9;
         }
      }

      this.method2366(0, var3 - 1);
      var6 = 0;
      var7 = lowNeighbor[0] * this.multiplier;

      for(var8 = 1; var8 < var3; ++var8) {
         if(step2Flags[var8]) {
            var9 = highNeighbor[var8];
            var10 = lowNeighbor[var8] * this.multiplier;
      ...
      float var14 = decibelStaticTable[var7];
